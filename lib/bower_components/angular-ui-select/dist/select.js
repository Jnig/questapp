...
(function () {
  ...
  .controller('uiSelectCtrl',
    ...
    $element.wrap('<div class="layer"></div>');
    var ctrl = this;
    ...
    ctrl.activate = function(initSearchValue) {
      ...
      if ($element.closest('.scroll').length > 0) {
        $element.parent().addClass('select-background');
        $timeout(function(){
            $element.closest('.scroll')[0].style.webkitTransform = "translate3d(0, 0, 0)";
        },200);
      }
    ...
    ctrl.close = function() {
      ...
      if (ctrl.open) {
        $element.closest('.scroll').css('transform', '');
        $element.parent().removeClass('select-background');
